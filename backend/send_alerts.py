...
async def send_daily_alerts():
    ...
    for sub in subscribers:
        try:
            ...
            if not all_jobs:
                print(f"  [ALERTS] No new jobs for {sub['email']} this run")
                continue
            
            # Create and send email
            html = format_job_alert_email(sub["name"], all_jobs, sub["email"])
            subject = f"🔥 {len(all_jobs)} new UK Finance Job{'s' if len(all_jobs) > 1 else ''} for you"
            ...
            if sent:
                ...
                print(f"  [ALERTS] Sent alert to {sub['email']} with {len(all_jobs)} jobs")
                successful += 1
            else:
    ...
    print(f"\n[ALERTS] Daily alert run complete: {successful} sent, {failed} failed")
